...
## [1.4.1] — Cross-Browser GitHub OAuth & Edge Certification Fix

### Fixed

- Fixed GitHub OAuth redirect URI handling for browser-specific extension callback URLs.
- The extension now uses the browser-generated OAuth redirect URI.
- The authorization code exchange now sends the same redirect URI to the backend.
- Added backend validation for allowed GitHub OAuth redirect URIs.
- Removed the fixed backend GitHub OAuth redirect URI dependency.
- Removed the extension manifest `key` field required for Microsoft Edge certification.

### Security

- Added an explicit backend allowlist for GitHub OAuth redirect URIs.
- Backend rejects redirect URIs that are not configured in `GITHUB_ALLOWED_REDIRECT_URIS`.
- GitHub OAuth client credentials remain stored in backend environment variables.

### Production Validation

- Validated backend `mvn clean test`.
- Validated production WXT extension build.
- Validated Chrome GitHub OAuth flow.
- Validated GitHub authorization callback.
- Promoted the OAuth fixes from `develop` to `main`.
- Deployed the updated backend to production.
- Created the `v1.4.1` release tag.
